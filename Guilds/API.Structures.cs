...
{
	public static PlayerReference fromPlayerInfo(ZNet.PlayerInfo playerInfo) => new() { id = playerInfo.m_userInfo.m_id.ToString().IsNullOrWhiteSpace() ? PlatformManager.DistributionPlatform.LocalUser.PlatformUserID.ToString() : playerInfo.m_userInfo.m_id.ToString().Contains("_") ? playerInfo.m_userInfo.m_id.ToString() : $"Steam_{playerInfo.m_userInfo.m_id.ToString()}", name = playerInfo.m_name ?? "" };
...
#if !API
	public static PlayerReference fromRPC(ZRpc? rpc) => rpc is null ? forOwnPlayer() : fromPlayerInfo(ZNet.instance.m_players.First(p => p.m_userInfo.m_id.ToString() == rpc.m_socket.GetHostName()));
#endif
